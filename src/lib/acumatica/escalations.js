// src/lib/acumatica/escalations.js
import AcumaticaService from "./auth/acumaticaService";

export async function writeT42({ orderType, orderNbr }) {
  if (!orderType || !orderNbr) {
    throw new Error("writeT42: orderType and orderNbr are required");
  }

  const {
    ACUMATICA_BASE_URL,
    ACUMATICA_CLIENT_ID,
    ACUMATICA_CLIENT_SECRET,
    ACUMATICA_USERNAME,
    ACUMATICA_PASSWORD,
  } = process.env;

  const restService = new AcumaticaService(
    ACUMATICA_BASE_URL,
    ACUMATICA_CLIENT_ID,
    ACUMATICA_CLIENT_SECRET,
    ACUMATICA_USERNAME,
    ACUMATICA_PASSWORD
  );

  const accessToken = await restService.getToken();

  // Only send what you want to change
  const payload = {
    custom: {
      Document: {
        AttributeSIXWEEKFF: { type: "CustomBooleanField", value: true },
      },
    },
  };

  // Key-addressed URL avoids ambiguous matches
  const url = `${restService.baseUrl}/entity/Default/24.200.001/SalesOrder?$filter=OrderType eq '${orderType}' and OrderNbr eq '${orderNbr}'`;
  
  const resp = await fetch(url, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
      Authorization: `Bearer ${accessToken}`,
    },
    body: JSON.stringify(payload),
  });

  const raw = await resp.text();
  let data;
  try {
    data = JSON.parse(raw);
  } catch {
    console.error("[writeT42] Non-JSON SalesOrder response:", raw);
    throw new Error("writeT42: Unexpected SalesOrder response (non-JSON)");
  }

  if (!resp.ok) {
    console.error("[writeT42] Acumatica SalesOrder error:", data);
    const msg = data?.message || "SalesOrder update failed";
    const err = new Error(`writeT42: ${msg}`);
    err.status = resp.status;
    err.data = data;
    throw err;
  }

  return data;
}

export async function writeT42EscalateNote ({noteID}) {
  if (!noteID) {
    throw new Error("writeT42EscalateNote: noteID is required");
  }

  const {
    ACUMATICA_BASE_URL,
    ACUMATICA_CLIENT_ID,
    ACUMATICA_CLIENT_SECRET,
    ACUMATICA_USERNAME,
    ACUMATICA_PASSWORD,
  } = process.env;

  const restService = new AcumaticaService(
    ACUMATICA_BASE_URL,
    ACUMATICA_CLIENT_ID,
    ACUMATICA_CLIENT_SECRET,
    ACUMATICA_USERNAME,
    ACUMATICA_PASSWORD
  );

  const accessToken = await restService.getToken();

  const payload = {
    Summary: { value: "Automation: Failed to reach customer at 6 week. Order added to 6 week report" },
    RelatedEntityNoteID: { value: noteID },
  }

  const url = `${restService.baseUrl}/entity/CustomEndpoint/24.200.001/Activity`;

  const resp = await fetch(url, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
      Authorization: `Bearer ${accessToken}`,
    },
    body: JSON.stringify(payload),
  });

  const raw = await resp.text();
  let data;
  try {
    data = JSON.parse(raw);
  } catch {
    console.error("[writeT42EscalateNote] Non-JSON Activity response:", raw);
    throw new Error("writeT42EscalateNote: Unexpected Activity response (non-JSON)");
  }

  if (!resp.ok) {
    console.error("[writeT42EscalateNote] Acumatica Activity error:", data);
    const msg = data?.message || "Activity update failed";
    const err = new Error(`writeT42EscalateNote: ${msg}`);
    err.status = resp.status;
    err.data = data;
    throw err;
  }

  return data;
}