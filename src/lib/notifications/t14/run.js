...
import { writeT14, writeT14Note } from '@/lib/acumatica/confirmations';
      ...
      try {
        ...
        try {
          ...
          if (!orderType) {
            ...
            // 3b) Best-effort Activity note (requires NoteID related to the Sales Order)
            try {
              const noteId = o?.noteId || o?.contact?.noteId || null;
              if (noteId) {
                await writeT14Note({ noteID: noteId });
              } else {
                console.warn('[T14][note skipped - no noteId]', o.orderNbr);
              }
            } catch (noteErr) {
              console.warn('[T14][note error]', o.orderNbr, noteErr?.message || noteErr);
            }
          }
        } catch (erpErr) {
          erpWriteErrors++;
